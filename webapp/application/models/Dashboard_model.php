...
class Dashboard_model extends CI_Model{
    ...
    public function login(){
        ...
        if($this->input->post('submit') ){
            ...
            $email = $this->input->post('clienteEmail');
            $pswd = $this->input->post('clienteSenha');
            
            if( !empty($pswd) OR !empty($email) ){

                $this->db->where('clienteEmail',$email);
                $user = $this->db->get('clientes');

                if($user->num_rows() > 0 ){

                    $this->db->where('clienteEmail', $user->row()->clienteEmail );
                    $cliente = $this->db->get('clientes');

                    if( $cliente->row()->clienteSenha == sha1($pswd) ){

                        //if( $user->row()->block != 1){

                            $this->session->set_userdata('cliente_id', $user->row()->clienteID );

                            // $this->db->where('clienteEmail',$user->row()->clienteEmail);
                            // $this->db->update('clientes',array('ultimoAcesso'=>date('Y-m-d H:i:s') ) );
    ...
    public function getVeiculo($placa){
        ...
        $this->db->where('veiculoPlaca', $placa);
        $resultado = $this->db->get('veiculos');
