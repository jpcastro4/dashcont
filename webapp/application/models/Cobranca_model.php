...
class Cobranca_model extends CI_Model{
    ...
    private function token = 'B3BC784833C34A10939F6E7A670004AF';
    private function loja = 'adm@grupodifference.com';

    public function abrirCobranca(){

        

        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, 'https://ws.pagseguro.uol.com.br/pre-approvals/request');
    ...
    public function produtos_veiculo($veiculoID){
        ...
        $this->db->select('*');
        $this->db->from('clientesProdutos');
        $this->db->where('clientesProdutos.veiculoID',$veiculoID);
        $this->db->join('produtos', 'produtos.produtoID = clientesProdutos.produtoID');
        $resultado = $this->db->get();
